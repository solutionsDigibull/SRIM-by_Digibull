...
async function main(): Promise<void> {
  ...
  try {
    // `require` is not available when tsx runs in ESM mode (--import tsx/esm).
    // Treat that the same as a missing package — the optional module is OSS-absent anyway.
    if (typeof require !== 'function') {
      throw Object.assign(new Error(`Cannot find module '${OPTIONAL_RUNTIME_MODULE}'`), {
  ...
  // Mutable holder so TaskService's connectivity probe can check browser SSE
  // clients even though BrowserApiServer is created after TaskService.
  // The probe closure captures this object by reference — when BrowserApiServer
  // assigns itself here, `hasConnectedClients()` automatically picks it up.
  const browserClients = { hasConnectedClients: () => false };
  ...
  const taskService = new TaskService(storage, {
    ...
    appPath,
    accomplishRuntime,
    // Counts BOTH Unix-socket RPC clients (Electron) AND HTTP SSE clients (browser).
    // Either presence means "UI is connected" — prevents the no-UI auto-deny policy
    // from firing when the user is only using the browser tab.
    rpcConnectivityProbe: { hasConnectedClients: () => rpc.hasConnectedClients() || browserClients.hasConnectedClients() },
    setProxyTaskId,
  });
  ...
  browserClients.hasConnectedClients = () => browserApi.hasConnectedClients();
