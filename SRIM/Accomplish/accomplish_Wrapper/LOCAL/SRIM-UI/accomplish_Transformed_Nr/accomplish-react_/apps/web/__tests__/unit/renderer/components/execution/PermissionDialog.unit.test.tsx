...
} from './permissionDialogTestHelpers';
...
describe('PermissionDialog', () => {
  describe('renders the inline card', () => {
    ...
    it('should show the question header for question type', () => {
      const onRespond = vi.fn();
      render(
        <PermissionDialog
          permissionRequest={createQuestionPermission({ header: 'AI follow-up' })}
          onRespond={onRespond}
        />,
      );

      expect(screen.getByText('AI follow-up')).toBeInTheDocument();
      expect(screen.getByText(/the ai paused here/i)).toBeInTheDocument();
    });
  });
  ...
  describe('question permission inline card', () => {
    it('should keep send answer disabled until the user responds', () => {
      const onRespond = vi.fn();
      render(
        <PermissionDialog
          permissionRequest={createQuestionPermission()}
          onRespond={onRespond}
        />,
      );

      expect(screen.getByTestId('permission-allow-button')).toBeDisabled();
    });

    it('should send the selected option when an answer chip is clicked', () => {
      const onRespond = vi.fn();
      render(
        <PermissionDialog
          permissionRequest={createQuestionPermission()}
      ...
      expect(onRespond).toHaveBeenCalledWith(true, ['Development'], undefined);
    });

    it('should send custom text when the user types a reply', () => {
      const onRespond = vi.fn();
      render(
        <PermissionDialog
          permissionRequest={createQuestionPermission()}
      ...
      fireEvent.change(screen.getByLabelText('Custom response'), {
        target: { value: 'Use staging for this task.' },
      });
      fireEvent.click(screen.getByTestId('permission-allow-button'));

      expect(onRespond).toHaveBeenCalledWith(true, [], 'Use staging for this task.');
    });
  });
});
