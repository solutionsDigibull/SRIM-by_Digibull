...
/** Viewport coordinates used to position the portaled dropdown. */
...
>(function SelectTrigger(
  { displayValue, placeholder, isOpen, error, testId, onClick, listboxId },
  ref,
) {
  return (
    <button
      ...
      className={`w-full rounded-md border bg-background px-3 py-2.5 text-sm text-left flex items-center justify-between transition-colors hover:border-muted-foreground/50 ${
...
>(function SelectDropdown(
  ...
  // Portaled to <body> so a clipping `overflow` ancestor (scrollable dialogs,
  // cards) can't trim the menu. Positioned with `fixed` against the trigger rect.
  ...
  const dropdown = (
    <motion.div
      ...
      id={listboxId}
      role="listbox"
      style={{ ...style, transformOrigin: position.openUp ? 'bottom' : 'top' }}
