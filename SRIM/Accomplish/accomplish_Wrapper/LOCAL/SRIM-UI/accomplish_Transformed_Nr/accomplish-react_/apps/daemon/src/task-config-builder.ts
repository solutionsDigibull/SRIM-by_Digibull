...
export async function onBeforeStart(
  ...
  // Build the spawn env via buildOpenCodeEnvironment so provider API keys are
  // injected as env vars (ANTHROPIC_API_KEY, OPENAI_API_KEY, GOOGLE_*, etc.).
  // Native providers read their key from the environment — NOT from the config
  // file or auth.json — so without this every anthropic/openai/google/xai task
  // starts with no credentials and fails. (The migration dropped this step; the
  // old PTY-era buildEnvironment helper used to do it.)
  const env: NodeJS.ProcessEnv = buildOpenCodeEnvironment(
    {
      OPENCODE_CONFIG: result.configPath,
      OPENCODE_CONFIG_DIR: path.dirname(result.configPath),
    },
    { apiKeys, taskId: ctx.taskId },
  );
