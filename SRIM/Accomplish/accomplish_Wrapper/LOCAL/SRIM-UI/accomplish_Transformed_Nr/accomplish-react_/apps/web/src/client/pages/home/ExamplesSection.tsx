        ...
        <div className="grid w-full grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {useCaseExamples.map((example, index) => (
            <motion.button
              ...
              className="group hover-lift flex h-[164px] flex-col justify-between rounded-[18px] border border-border bg-background/70 px-4 py-3 text-left backdrop-blur transition-colors hover:border-primary/20 hover:bg-background/90"
            >
              <div className="flex items-start justify-between w-full">
                <span className="w-[120px] whitespace-pre-line font-sans text-[14px] leading-[18px] tracking-[-0.28px] text-foreground">
                  {example.title}
                </span>
                <span className="shrink-0 opacity-0 transition-all duration-200 translate-y-1 group-hover:translate-y-0 group-hover:opacity-100 group-active:translate-y-0 group-active:opacity-100 -scale-y-100 rotate-180">
              ...
              <div className="flex items-center gap-[2px]">
                {example.icons.map((domain) => (
                  <div
                    key={domain}
                    className="flex shrink-0 items-center rounded-[8px] bg-popover/80 p-[3.25px]"
                  >
